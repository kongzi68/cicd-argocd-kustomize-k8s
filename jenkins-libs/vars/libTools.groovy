...
def delDeploySVCName(List listDuplicateSVC, String retainSVC){
    /*
        ...
        * listDuplicateSVC, 都是同一个jar包的服务名称列表，比如 bar-svc-api-app,bar-svc-api-app-master,bar-svc-offline-app 用的同一个jar包
        * retainSVC，需要保留的服务名称
    */
    _svc_list = []
...
def splitImage(imageName) {
    /*
        比如镜像为：iamIPaddr:11180/libs/elasticsearch/elasticsearch:7.17.3
        拆分为：[iamIPaddr:11180, libs, libs/elasticsearch/elasticsearch, elasticsearch%252Felasticsearch, elasticsearch, 7.17.3]
    */
    retList = []
    if (imageName != '') {
